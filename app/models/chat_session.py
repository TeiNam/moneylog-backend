"""
ChatSession SQLAlchemy 모델.

ledger 스키마에 속하는 AI 채팅 세션 테이블을 정의한다.
FK 제약조건 없이 논리적 FK만 사용한다 (PostgreSQL 가이드라인 준수).
월별 RANGE 파티셔닝이 적용되어 created_at 기준으로 파티션된다.
"""

from uuid import UUID, uuid4

from sqlalchemy import Index, PrimaryKeyConstraint, String
from sqlalchemy.dialects.postgresql import UUID as PG_UUID
from sqlalchemy.orm import Mapped, mapped_column

from app.models.base import Base, created_at, nullable_timestamp


class ChatSession(Base):
    """AI 채팅 세션 모델 (ledger.ai_chat_sessions 테이블)."""

    __tablename__ = "ai_chat_sessions"
    __table_args__ = (
        # 복합 PK: 파티션 키(created_at)는 반드시 PK에 포함되어야 함
        PrimaryKeyConstraint("id", "created_at"),
        Index("idx_ai_chat_sessions_user_id_created_at", "user_id", "created_at"),
        {
            "schema": "ledger",
            "comment": "AI 채팅 세션 테이블",
            "postgresql_partition_by": "RANGE (created_at)",
        },
    )

    # UUID PK (복합 PK의 일부, created_at과 함께 구성)
    id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True), default=uuid4,
    )
    # 논리적 FK: auth.users.id (FK 제약 없음)
    user_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        nullable=False,
        comment="논리적 FK → auth.users.id",
    )
    title: Mapped[str | None] = mapped_column(
        String(200), nullable=True, comment="세션 제목",
    )
    created_at: Mapped[created_at]
    updated_at: Mapped[nullable_timestamp]
